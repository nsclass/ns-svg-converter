...
public class ColorQuantizator {
    ...
    public int[][] colorQuantization(ImageData imageData, byte[][] palette, ImageConvertOptions options) {
        float minColorRatio = options.getMinColorRatio();
        int cycles = options.getColorQuantCycles();

        // Creating indexed color array indexedDataArray which has a boundary filled with -1 in every direction
    ...
    private static void colorQuantizationIteration(int loopIdx,
        ...
        if (loopIdx > 0) {
            ...
            for (int k = 0; k < palette.length; k++) {
                ...
                float ratio = (float) ((double) (paletteAcc[k][4]) / (double) (imageData.getWidth() * imageData.getHeight()));
    ...
    private static void rowColorQuantizationOnPixel(int row,
                                                    int[][] indexedDataArray,
                                                    long[][] paletteAcc,
                                                    ImageData imageData,
                                                    byte[][] palette) {
    ...
    private static final double[][] GAUSSIAN_KERNEL_FOR_BLUR = {{0.27901, 0.44198, 0.27901}, {0.135336, 0.228569, 0.272192, 0.228569, 0.135336}, {0.086776, 0.136394, 0.178908, 0.195843, 0.178908, 0.136394, 0.086776},
    ...
    private static ImageData blur(ImageData imageData, float rad, float del) {
        ...
        for (int row = 0; row < imageData.getHeight(); row++) {
            for (int col = 0; col < imageData.getWidth(); col++) {
                ...
                int d = Math.abs(imageRawData2[idx] - imageData.dataAt(idx)) +
        ...
        return new ImageData(imageData.getWidth(),
                imageData.getHeight(),
                imageRawData2);
