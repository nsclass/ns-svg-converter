...
import com.acrocontext.image.svg.process.ColorQuantizator;
import com.acrocontext.image.svg.process.InterNodeGenerator;
import com.acrocontext.image.svg.process.LayerGenerator;
import com.acrocontext.image.svg.process.PaletteGenerator;
import com.acrocontext.image.svg.process.ScanPathGenerator;
...
public class ImageSvgConverter {
  ...
  public String convertImageToSVG(
    ...
    return imageToSVG(operationManager, bufferedImage).execute(convertCtx).getSvgString();
  }
  ...
  private OperationManager<ImageSvgConvertCtx> loadImageData(
      OperationManager<ImageSvgConvertCtx> operationManager, String filename) {

    operationManager.addOperation(
        "loading image data",
        (ImageSvgConvertCtx ctx) -> {
          try {
  ...
  private OperationManager<ImageSvgConvertCtx> imageToSVG(
      OperationManager<ImageSvgConvertCtx> operationManager, String filename) {
  ...
  private OperationManager<ImageSvgConvertCtx> imageToSVG(
      OperationManager<ImageSvgConvertCtx> operationManager, BufferedImage bufferedImage) {

    operationManager.addOperation(
        "loading an image from buffer",
        ctx -> {
  ...
  private OperationManager<ImageSvgConvertCtx> imageDataToSVG(
      OperationManager<ImageSvgConvertCtx> operationManager) {
  ...
  private OperationManager<ImageSvgConvertCtx> imageDataToTraceData(
      OperationManager<ImageSvgConvertCtx> operationManager) {
    // 0. Generate palette
    ...
    operationManager.addOperation(
        ...
        ctx -> {
          int[][] paletteColors =
              colorQuantizator.colorQuantization(
                  ctx.getImageData(), ctx.getPalette(), ctx.getOptions());
    ...
    operationManager.addOperation(
        "batch create scan paths",
        ctx -> {
          ctx.batchPathScan =
              scanPathGenerator.createBatchScanPath(
                  ctx.getRawLayers(), ctx.getOptions().getPathOmit());
          return ctx;
        });
    ...
    operationManager.addOperation(
        ...
        ctx -> {
          ctx.batchInterNodes =
              interNodeGenerator.createInterNodeListLayers(ctx.getBatchPathScan());
          return ctx;
        });
    ...
    operationManager.addOperation(
        "batch tracing",
        ctx -> {
          TracePathLayers[] layers =
              tracePathGenerator.createBatchTracePathList(
                  ctx.getBatchInterNodes(),
                  ctx.getOptions().getLThreshold(),
                  ctx.getOptions().getQThreshold());
  ...
  private void generateSvgPathFromTracePaths(
      StringBuilder sb,
      String desc,
      TracePath segments,
      String colorStr,
    ...
    sb.append("<path ")
        ...
        .append(" ")
        .append(segments.valueAt(0, 2) * scale)
        .append(" ");
  ...
  private String getSvgString(IndexedImage indexedImage, ImageConvertOptions options) {
    // SVG start
    int w = (int) (indexedImage.width * options.getScale()),
        h = (int) (indexedImage.height * options.getScale());

    String viewBoxOrViewPort =
        options.getViewBox() != 0
            ? "viewBox=\"0 0 " + w + " " + h + "\" "
            : "width=\"" + w + "\" height=\"" + h + "\" ";
    StringBuilder svgStringBuilder =
        new StringBuilder(
    ...
    for (int layerIdx = 0; layerIdx < indexedImage.layers.length; layerIdx++) {
      ...
      for (int pathCount = 0; pathCount < indexedImage.pathSizeAtIdx(layerIdx); pathCount++) {
        ...
        double label =
            (indexedImage.valueAtIdx(layerIdx, pathCount, 0, 2) * w)
                + indexedImage.valueAtIdx(layerIdx, pathCount, 0, 1);
