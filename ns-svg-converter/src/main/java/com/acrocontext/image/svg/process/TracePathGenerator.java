...
public class TracePathGenerator {
  ...
  private static TracePath createTracePath(InterNodeList path, float lThreshold, float qThreshold) {
    ...
    while (pathIndex < pathLength) {
      ...
      while (((path.getInterNodes()[sequenceEnd].getPoint()[2] == sequenceType1)
              || (path.getInterNodes()[sequenceEnd].getPoint()[2] == sequenceType2)
              || (sequenceType2 == -1))
          && (sequenceEnd < (pathLength - 1))) {
        if ((path.getInterNodes()[sequenceEnd].getPoint()[2] != sequenceType1)
            && (sequenceType2 == -1)) {
  ...
  private static ArrayList<Double[]> fitSequence(
      InterNodeList path, float lThreshold, float qThreshold, int seqStart, int seqEnd) {
    ...
    double vx =
        (path.valueAtWithSequenceIdx(seqEnd, 0) - path.valueAtWithSequenceIdx(seqStart, 0)) / tl;
    double vy =
        (path.valueAtWithSequenceIdx(seqEnd, 1) - path.valueAtWithSequenceIdx(seqStart, 1)) / tl;
    ...
    while (pathIndex != seqEnd) {
      ...
      double dist2 =
          ((path.valueAtWithSequenceIdx(pathIndex, 0) - px)
                  * (path.valueAtWithSequenceIdx(pathIndex, 0) - px))
              + ((path.valueAtWithSequenceIdx(pathIndex, 1) - py)
                  * (path.valueAtWithSequenceIdx(pathIndex, 1) - py));
      if (dist2 > lThreshold) {
        curvePass = false;
      }
    ...
    double t = (fitPoint - seqStart) / tl,
        ...
        t3 = t * t;
    double cpx =
        (((t1 * path.valueAtWithSequenceIdx(seqStart, 0))
                    + (t3 * path.valueAtWithSequenceIdx(seqEnd, 0)))
                - path.valueAtWithSequenceIdx(fitPoint, 0))
            / -t2;
    double cpy =
        (((t1 * path.valueAtWithSequenceIdx(seqStart, 1))
                    + (t3 * path.valueAtWithSequenceIdx(seqEnd, 1)))
                - path.valueAtWithSequenceIdx(fitPoint, 1))
            / -t2;
  ...
  private static TracePath[] batchTracePaths(
      InterNodeListLayers interNodePaths, float lThreshold, float qThreshold) {
