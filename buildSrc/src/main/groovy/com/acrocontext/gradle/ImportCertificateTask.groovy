...
class ImportCertificateTask extends DefaultTask {
    ...
    ImportCertificateTask() {
        description = "Creates a truststore with the self signed certificate imported"
        group = "certificate"
    }
    ...
    @TaskAction
    def exec() {
        if (!project.file(trustStore).exists()) {
            project.exec {
                commandLine = ['keytool', '-import',
