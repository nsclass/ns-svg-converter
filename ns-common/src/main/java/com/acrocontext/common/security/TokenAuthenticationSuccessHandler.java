...
public class TokenAuthenticationSuccessHandler implements ServerAuthenticationSuccessHandler {
    ...
    @Autowired
    public TokenAuthenticationSuccessHandler(TokenProvider tokenProvider,
    ...
    public Mono<Void> onAuthenticationSuccess(WebFilterExchange webFilterExchange,
        ...
        return applicationSettingsService.getApplicationSettingsInAsync()
                .flatMap(settings -> processLogin(webFilterExchange, token,
                        settings.getTokenSettings().getExpireInSeconds()));
    }

    private Mono<Void> processLogin(WebFilterExchange webFilterExchange,
                                    String token, int tokenExpire) {
        UserTokenData userTokenData = new UserTokenData(token,
                tokenExpire);

        return objectMapper.toJson(userTokenData)
                .flatMap(str -> {
